...
class UserController {
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      logger.error(
        `error occured unable to create a user ${JSON.stringify(error)}`
      );
  ...
  async list(req: Request, res: Response) {
    try {
      ...
      return res
        .status(200)
        .send(responsesHelper.output(200, data.message, data.data, data.meta));
    } catch (error) {
      logger.error(
        `error occured unable to list users ${JSON.stringify(error)}`
      );
      return res.status(500).send(responsesHelper.error(500, `${error}`));
    }
  }
